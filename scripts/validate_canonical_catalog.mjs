...
const reel6 = reels.find((reel) => reel.reel_id === '0006');
...
if (reel5.status !== 'qc_passed_drive_verified' || reel5.duration_seconds < 55 || reel5.duration_seconds > 65 || !reel5.drive_file_id || !reel5.source_metadata_file_id || reel5.qc_file_id === 'PENDING_FINAL_DRIVE_QC_UPLOAD') fail('Reel 0005 completion contract is invalid.');
if (reel6.status !== 'qc_passed_drive_verified' || reel6.duration_seconds < 55 || reel6.duration_seconds > 65 || !reel6.drive_file_id || !reel6.source_metadata_file_id || reel6.qc_file_id === 'FINAL_QC_PENDING_UPLOAD') fail('Reel 0006 completion contract is invalid.');
const reel7 = reels.find((reel) => reel.reel_id === '0007');
if (reel7.status !== 'planned_research_pending' || reel7.concept_key === reel6.concept_key) fail('Reel 0007 must be a concept-distinct research-pending slot.');
if (state.state.completed_reels !== 6 || state.state.next_reel !== '0007' || state.state.last_verified_reel !== '0006' || state.state.retry_queue.length !== 0) fail('Canonical production checkpoint is invalid.');
if (!Array.isArray(state.state.completed_concept_keys) || state.state.completed_concept_keys.length !== 6 || !state.state.completed_concept_keys.includes(reel6.concept_key) || new Set(state.state.completed_concept_keys).size !== 6) fail('Reel 0006 concept completion contract is invalid.');
console.log('Canonical catalog validation passed: 3000 unique reels, 100 complete batches, Reels 0001–0006 verified, concept-distinct Reel 0007 next.');
