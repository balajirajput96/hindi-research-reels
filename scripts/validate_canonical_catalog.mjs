...
if (reel6.status !== 'planned_research_pending' || reel6.concept_key === reel5.concept_key) fail('Reel 0006 must be a concept-distinct research-pending slot.');
if (state.state.completed_reels !== 5 || state.state.next_reel !== '0006' || state.state.last_verified_reel !== '0005' || state.state.retry_queue.length !== 0) fail('Canonical production checkpoint is invalid.');
if (!Array.isArray(state.state.completed_concept_keys) || state.state.completed_concept_keys.length !== 5 || !state.state.completed_concept_keys.includes(reel5.concept_key) || new Set(state.state.completed_concept_keys).size !== 5) fail('Reel 0005 concept completion contract is invalid.');
console.log('Canonical catalog validation passed: 3000 unique reels, 100 complete batches, Reels 0001–0005 verified, concept-distinct Reel 0006 next.');
