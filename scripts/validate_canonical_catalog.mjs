...
if (reel2.status !== 'qc_passed_drive_verified' || reel2.duration_seconds < 55 || reel2.duration_seconds > 65) fail('Reel 0002 completion contract is invalid.');
if (reel3.status !== 'planned_research_pending' || state.state.retry_queue.length !== 0) fail('Reel 0003 next-research contract is invalid.');
if (state.state.completed_reels !== 2 || state.state.next_reel !== '0003') fail('Canonical production checkpoint is invalid.');
console.log('Canonical catalog validation passed: 3000 unique reels, 100 complete batches, Reels 0001–0002 verified, Reel 0003 staged for research.');
